...
def img2char(convertor: Convertor, font_size=7, type_of_chara=TYPE_OF_CHARACTER.alpha):
  ...
  # 创建绘制对象
  font = ImageFont.truetype(font=font_path(), size=font_size, encoding="unic")
  (font_width, font_height) = font.getsize('A') if type_of_chara == TYPE_OF_CHARACTER.alpha else font.getsize('1')
  ...
  for y in range(height):
    for x in range(width):
      if x % font_width == 0 and y % font_height == 0:
        draw.text((x, y), table[randint(0, table_len - 1)], pix[x, y], font)
        pix_count += 1
