...
const search: Handler = async ({ recv, resp, error }) => {
  try {
    const march = recv.message.match(/^hs_search\s+([^\s\\\/#!\+-]+)/iu);
    ...
    const response = await axios.post(
      'https://api2.iyingdi.com/hearthstone/card/search/vertical',
      form,
      {
        responseType: 'json',
        headers: {
    ...
    cards.forEach(i => {
      resp.addMsg(
        segment.text(
          annotation(`${i.cname}`, i.seriesName + ' (' + i.seriesAbbr + ')')
        )
      );
